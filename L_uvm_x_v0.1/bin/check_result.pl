...
if( !-e "vcs.log" ){
	...
	$lin_vio  = `grep "violation" vcs.log | grep -v "violation :" | wc -l`;
    
	#$lin_war  = `grep "WARNING.*FAILURE" vcs.log | wc -l`;
 
	#if( ($lin_time + $lin_err + $lin_ng + $lin_ERR + $lin_sva + $lin_war + $lin_fatal) == 0 ) {
	if( (($lin_compare==0) + $lin_time + $lin_err + $lin_ng + $lin_ERR + $lin_sva + $lin_fatal + $lin_vio) == 0 ) {#FHM 
		...
		if( ($lin_vio) != 0 ){
 			print RESULT_FILE " NG:timing violation error";
		}
